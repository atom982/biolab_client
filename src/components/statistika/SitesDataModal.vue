...
export default {
  ...
  methods: {
    ...
    ok() {
      ...
      if (this.labassay.analit.length > 30) {
        bus.$emit("LabAssayLength", this.labassay.analit);
        bus.$emit("LabAssayEdit");
      } else {
        bus.$emit("LabAssayEdit");
      }
